...
export function DriverSidebar({ user }: { user: { name?: string | null; email?: string | null; role: string } }) {
  ...
  return (
    <aside className="w-52 flex-shrink-0 bg-emerald-900 text-white flex flex-col">
      <div className="p-4 border-b border-white/10">
        <div className="flex items-center gap-2">
          <Shield className="h-5 w-5 text-emerald-300" />
          <span className="font-bold">Nexum</span>
        </div>
        <div className="text-xs text-emerald-300 mt-1">{roleName(user.role)}</div>
      </div>
      ...
      <div className="p-3 border-t border-white/10">
        ...
        <button onClick={() => signOut({ callbackUrl: '/login' })}
          className="mt-2 flex items-center gap-2 text-xs text-emerald-300 hover:text-white transition-colors">
