...
export function AdminSidebar({ user }: { user: { name?: string | null; email?: string | null; role: string } }) {
  ...
  return (
    <aside className="w-56 flex-shrink-0 bg-[#1B3A6B] text-white flex flex-col">
      {/* Brand */}
      <div className="p-4 border-b border-white/10">
        ...
        <div className="text-xs text-blue-200 mt-1">{roleName(user.role)}</div>
      </div>
      ...
      <div className="p-3 border-t border-white/10">
        ...
        <button
          ...
          className="mt-2 flex items-center gap-2 text-xs text-blue-200 hover:text-white transition-colors"
        >
